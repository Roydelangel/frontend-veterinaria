        ...
        <div className='flex justify-center'>
          ...
          { msg && <Alerta alerta={alerta} />}
            <form onSubmit={handleSubmit}>
              <div className='my-3'> 
                <label className='text-gray-600 uppercase font-bold'>Nombre</label>
                <input type='text' className='border bg-gray-50 w-full p-2 mt-5 rounded-lg' name='nombre'
                value={perfil.nombre || ''} onChange={e => setPerfil({
                  ...perfil, 
                  [e.target.name]: e.target.name})}/>
              </div>
              
              <div className='my-3'> 
                <label className='text-gray-600 uppercase font-bold'>Email</label>
                <input type='text' className='border bg-gray-50 w-full p-2 mt-5 rounded-lg' name='email'
                value={perfil.email || ''} onChange={e => setPerfil({
                  ...perfil, 
                  [e.target.name]: e.target.name})}/>
              </div>
              
              <div className='my-3'> 
                <label className='text-gray-600 uppercase font-bold'>Sitio Web</label>
                <input type='text' className='border bg-gray-50 w-full p-2 mt-5 rounded-lg' name='web'
                  value={perfil.web || ''} onChange={e => setPerfil({
                  ...perfil, 
                  [e.target.name]: e.target.name})}/>
              </div>
              
              <div className='my-3'> 
                <label className='text-gray-600 uppercase font-bold'>Telefono</label>
                <input type='text' className='border bg-gray-50 w-full p-2 mt-5 rounded-lg' name='telefono'
                  value={perfil.telefono || ''} onChange={e => setPerfil({
                  ...perfil, 
                  [e.target.name]: e.target.name})}/>
              </div>
